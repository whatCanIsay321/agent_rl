...
#
# generate_samples()
def get_action_log_probs(model, input_ids, attention_mask, num_actions):
    # 计算策略模型输出token的概率
...
def generate_exprience():
    ...
    with torch.no_grad():
                ...
                for i, (reward_func, reward_tokenizer) in enumerate(
                        zip(reward_funcs, reward_tokenizers)
                ):
                    if isinstance(reward_func, PreTrainedModel):
                        with torch.inference_mode():
                            ...
                            rewards_per_func[i] = reward_func(
                                **reward_model_inputs.to(args.device)).logits.squeeze(-1)
                ...
                if not args.reward_weights:
                    args.reward_weights = [1.0] * len(reward_funcs)
                if len(args.reward_weights) != len(reward_funcs):
                    raise ValueError("The number of reward weights must be equal to the number of reward functions.")
                # 乘以各个奖励函数的权重
                rewards = rewards_per_func * torch.tensor(args.reward_weights, dtype=torch.float32,
